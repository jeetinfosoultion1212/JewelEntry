...
// Generate QR Code for UPI payment - using default UPI ID since upi_id column doesn't exist
$upi_id = '9810359334@ybl'; // Default UPI ID since the column doesn't exist in Firm table
$upi_data = "upi://pay?pa=$upi_id&am=" . $saleDetails['grand_total'];

// Define UPI QR code directory and ensure it exists
$upi_qr_dir = 'qrcodes/upi/';
if (!is_dir($upi_qr_dir)) {
    mkdir($upi_qr_dir, 0777, true);
}

// Use invoice_no for UPI QR code filename
$upi_qr_code_file = $upi_qr_dir . 'upi_qr_code_' . $invoice_no . '.png';
QRcode::png($upi_data, $upi_qr_code_file, QR_ECLEVEL_L, 5);
...
$gst_amount = $saleDetails['gst_amount'];
...
$isGSTApplicable = $saleDetails['is_gst_applicable'] == 1;
$stateCode = $customerState == $firmDetails['State'] ? '07' : '99';

// Calculate GST breakdown
if ($isGSTApplicable && $stateCode == '07') {
    $cgst = $gst_amount / 2;
    $sgst = $gst_amount / 2;
    $igst = 0;
} elseif ($isGSTApplicable) {
    $cgst = 0;
    $sgst = 0;
    $igst = $gst_amount;
} else {
...
// Generate QR code for digital bill page
$digital_bill_url = "https://yourdomain.com/digital_bill.php?sale_id=" . $saleDetails['id'];
$digital_bill_qr_dir = 'qrcodes/digital_bill/';
if (!is_dir($digital_bill_qr_dir)) {
    mkdir($digital_bill_qr_dir, 0777, true);
}
$digital_bill_qr_file = $digital_bill_qr_dir . 'digital_bill_qr_' . $saleDetails['id'] . '.png';
QRcode::png($digital_bill_url, $digital_bill_qr_file, QR_ECLEVEL_L, 4);
?>
...
<head>
    ...
    <title>Jewelry Invoice - Compact Design</title>
    ...
    <style>
        ...
        .invoice-container {
            width: 210mm;
            min-height: auto; /* Changed from fixed 290mm to auto */
            background: white;
        ...
        .elegant-header {
            background: linear-gradient(135deg, #fffbf0 0%, #f7f3e7 50%, #f0ead6 100%);
            border-bottom: 2px solid #d4af37;
            position: relative;
            overflow: hidden;
            margin: -4mm -4mm 2mm -4mm; /* Reduced negative margins */
            padding: 16mm 6mm 10mm 6mm; /* Increased header height */
        }
        ...
        .invoice-card {
            background: linear-gradient(135deg, #fffbf0 0%, #fef7e7 100%);
        ...
        .footer-elegant {
            ...
            color: white;
            position: relative;
            margin: 2mm -4mm -4mm -4mm; /* Reduced negative margins */
            padding: 3mm 2mm;
        }
        ...
        /* Fixed height table - MAINTAINED as requested */
        .table-fixed-height {
            height: 340px;
            display: flex;
        ...
        .table-fixed-height thead, .table-fixed-height tfoot {
            flex: 0 0 auto;
            display: table;
            width: 100%;
            table-layout: fixed;
        ...
        .table-section-border {
            /* Remove border from wrapper, only use for spacing/background */
            border: none;
            border-radius: 0;
            overflow: hidden;
            margin-bottom: 1rem;
        ...
        @page { 
            size: A4; 
            margin: 5mm; /* Added proper page margins */
        }
        @media print {
            body { 
                background: white !important;
                margin: 0;
                padding: 0 !important; /* Removed body padding for print */
                font-size: 11px;
            }
            .invoice-container {
                ...
                padding: 2mm !important; /* Reduced padding significantly */
                border: 1px solid #d4af37 !important; /* Thinner border */
            ...
            .elegant-header {
                margin: -2mm -2mm 2mm -2mm !important; /* Minimal negative margins */
                padding: 3mm !important; /* Reduced padding */
            }
            .footer-elegant {
                margin: 2mm -2mm -2mm -2mm !important; /* Minimal negative margins */
                padding: 2mm 1mm !important; /* Reduced padding */
            }
            
            /* Keep table height fixed in print too */
            .table-fixed-height {
                height: 290px !important; /* Slightly reduced for print */
            }
            .table-fixed-height tbody {
                overflow-y: visible !important; /* Allow content to show in print */
            }
        ...
        @media print {
          .terms-section { font-size: 9px !important; }
        }
    </style>
</head>
<body class="p-2">
    <div id="invoice-content" class="invoice-container mx-auto relative">
        ...
        <div class="text-center py-1 relative">
            ...
            <h2 class="text-base font-bold text-gray-800 mb-1">TAX INVOICE</h2>
        ...
        <div class="grid grid-cols-2 gap-3 mb-1">
            ...
            <div class="info-card invoice-card p-3">
                <h3 class="text-xs font-semibold text-gray-800 mb-2 border-b border-gray-200 pb-1">
                    <span class="golden-accent">📄</span> Invoice Details
                </h3>
                <div class="space-y-1">
                    <div class="grid grid-cols-2 gap-2 text-xs">
                        <div>
                        ...
                        <div>
                            ...
                            <p class="<?= $saleDetails['payment_status'] == 'Paid' ? 'text-green-600' : ($saleDetails['payment_status'] == 'Partial' ? 'text-orange-600' : 'text-red-600') ?> font-semibold">
        ...
        <div class="grid grid-cols-3 gap-3 mb-1">
            ...
            <div class="amount-section p-1">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="ml-2">
                        <div class="qr-frame">
                            <img src="<?= $upi_qr_code_file ?>" alt="UPI QR Code" class="w-12 h-12 bg-gray-100 rounded flex items-center justify-center" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="amount-section p-1">
                ...
                <div class="space-y-1 text-xs">
                    ...
                    <?php if ($isGSTApplicable && $gst_amount > 0): ?>
                        <?php if ($stateCode == '07'): ?>
                            <div class="flex justify-between">
                                <span class="text-gray-600">CGST (9%):</span>
                                <span class="font-medium">₹<?= number_format($cgst, 2) ?></span>
                            </div>
                            <div class="flex justify-between mb-2">
                                <span class="text-gray-600">SGST (9%):</span>
                                <span class="font-medium">₹<?= number_format($sgst, 2) ?></span>
                            </div>
                        <?php else: ?>
                            <div class="flex justify-between mb-2">
                                <span class="text-gray-600">IGST (18%):</span>
                                <span class="font-medium">₹<?= number_format($igst, 2) ?></span>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
        ...
        <div class="terms-section p-1 mb-1 text-xs rounded border border-gray-200 bg-gray-50" style="font-size:10px; page-break-inside: avoid;">
            ...
            <ul class="list-disc pl-4 space-y-0.5">
                ...
                <li>Ensure to check the items and bill before leaving the counter.</li>
        ...
        <div class="footer-elegant flex items-center justify-between">
            ...
            <div class="qr-frame inline-block">
                <img src="<?= $digital_bill_qr_file ?>" alt="Digital Bill QR Code" class="w-10 h-10 bg-white rounded flex items-center justify-center" />
            </div>
        </div>
    </div>
    <!-- Action Buttons -->
    ...
    <script>
        function exportToPDF() {
            ...
            html2canvas(element, {
                ...
                pdf.save('jewelry-invoice-compact.pdf');
