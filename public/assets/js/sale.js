...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Fetch and display active schemes
  fetchAndDisplayActiveSchemes()
})

// Function to fetch and display active schemes
function fetchAndDisplayActiveSchemes() {
  fetch("sale-entry.php?action=getActiveSchemes")
    .then((res) => res.json())
    .then((data) => {
      if (data.success && data.schemes && data.schemes.length > 0) {
        const marquee = document.getElementById("activeSchemesMarquee")
        const textSpan = document.getElementById("schemesMarqueeText")
        const schemeTexts = data.schemes.map(
          (s) =>
            `🎁 <b>${s.scheme_name || s.title}</b>: Min Purchase ₹${s.min_purchase_amount} | Ends: ${s.end_date ? new Date(s.end_date).toLocaleDateString() : ""}`
        )
        textSpan.innerHTML = schemeTexts.join(" &nbsp; | &nbsp; ")
        marquee.classList.remove("hidden")
      }
    })
    .catch((err) => {
      // Optionally hide marquee on error
      const marquee = document.getElementById("activeSchemesMarquee")
