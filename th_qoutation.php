...
if (isset($_GET['invoice_no'])) {
    ...
    echo "<div class='bg-danger-light text-danger p-6 rounded-lg text-center shadow-elegant'>
            <h2 class='font-serif text-2xl mb-3'>Quotation Information Missing</h2>
            <p>Please provide a valid quotation number or ID.</p>
...
if ($invoice_result->num_rows == 0) {
    echo "<div class='bg-danger-light text-danger p-6 rounded-lg text-center shadow-elegant'>
            <h2 class='font-serif text-2xl mb-3'>Quotation Not Found</h2>
            <p>The requested quotation was not found or you don't have permission to view it.</p>
            <a href='dashboard.php' class='inline-block mt-4 bg-primary text-white px-4 py-2 rounded'>
                Return to Dashboard
            </a>";
    exit();
...
$is_gst_applicable = isset($invoice['is_gstapplicable']) ? (int)$invoice['is_gstapplicable'] : 0;
...
$gst_rate = 0.03; // 3% GST
$gst_amount = $is_gst_applicable ? ($subtotal * $gst_rate) : 0;
$grand_total = $subtotal + $gst_amount;

// Close database connection
$conn->close();

// Generate a valid date for quotation
$quotation_valid_date = date('d-M-Y', strtotime('+30 days'));
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title><?php echo $is_gst_applicable ? "Proforma Invoice" : "Quotation"; ?> - <?php echo $invoice['invoice_no']; ?></title>
  <style>
...
<body>
  <div class="receipt">
    <!-- Watermark -->
    <div class="watermark"><?php echo $is_gst_applicable ? "PROFORMA" : "QUOTATION"; ?></div>
    ...
    <!-- Document Type Label -->
    <div class="doc-type">
      <?php echo $is_gst_applicable ? "PROFORMA INVOICE" : "QUOTATION"; ?>
    </div>
    <div class="valid-until">
      Valid until: <?php echo $quotation_valid_date; ?>
    </div>

    <!-- Invoice & Customer -->
    <table class="details">
      <tr>
        <td><strong><?php echo $is_gst_applicable ? "Proforma No:" : "Quotation No:"; ?></strong> <?php echo htmlspecialchars($invoice['invoice_no']); ?></td>
    ...
    <table class="items">
      ...
      <tbody>
        ...
        <tr>
          <td colspan="6" style="text-align:center;">No items found for this quotation</td>
        </tr>
        <?php endif; ?>
      </tbody>
    </table>
    ...
    <table class="summary">
      ...
      <tr class="total"><td class="label">Estimated Total</td><td class="value"><?php echo formatIndianRupee($grand_total); ?></td></tr>
    </table>
    ...
    <div class="note">
      <p><strong>Note:</strong> This is <?php echo $is_gst_applicable ? "a proforma invoice" : "a quotation"; ?> and not a tax invoice.</p>
      <p><em>Price may fluctuate based on market gold rate at the time of actual purchase.</em></p>
      <p>Quote valid for 30 days from date of issue.</p>
      <p>To confirm your order, please contact us at <?php echo htmlspecialchars($invoice['FirmPhoneNumber']); ?></p>
      <p>Follow us: FB/IG @<?php echo strtolower(str_replace(' ', '', $invoice['FirmName'])); ?></p>
    </div>

    <div class="footer">
      <p style="text-align:center; margin-top:10px; font-style:italic; font-size:8px;">Thank you for your interest in our jewelry!</p>
      <p class="no-print" style="text-align:center; margin-top:15px;">
        <button onclick="window.print()" style="padding:8px 15px; background:#0066cc; color:white; border:none; border-radius:4px; cursor:pointer;">
          Print <?php echo $is_gst_applicable ? "Proforma Invoice" : "Quotation"; ?>
        </button>
