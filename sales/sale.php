...
if (isset($_GET['action'])) {
   ...
   if ($action == 'searchCustomers') {
       ...
       while ($row = $result->fetch_assoc()) {
           ...
           $ordersQuery = "SELECT id, order_number, '' as item_name, advance_amount, grand_total as total_estimated, order_status as status 
                          FROM jewellery_customer_order 
                          WHERE customer_id = ? AND order_status = 'ready' AND advance_amount > 0";
   ...
   if ($action == 'getCustomerAdvancePayments') {
       ...
       $sql = "SELECT o.id, o.order_number, 
                      (SELECT item_name FROM jewellery_order_items WHERE order_id = o.id LIMIT 1) as item_name, 
                      o.advance_amount, o.advance_used, o.created_at,
                      (o.advance_amount - COALESCE(o.advance_used, 0)) as available_amount
               FROM jewellery_customer_order o
               WHERE o.customer_id = ? 
                 AND o.order_status IN ('ready', 'in progress')
                 AND o.advance_amount > COALESCE(o.advance_used, 0)
