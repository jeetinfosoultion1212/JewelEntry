document.addEventListener('DOMContentLoaded', function() {
  ...
  stats.forEach(stat => {
      ...
      function loadSuppliers(selectSupplierId = null) {
          fetch('stock_functions.php', {
              method: 'POST',
              headers: {
                  'Content-Type': 'application/x-www-form-urlencoded',
          ...
          .then(data => {
              if(Array.isArray(data)) {
                  updateSuppliersList(data, selectSupplierId);
              }
          })
          .catch(handleError);
      }
      ...
      function updatePurityDetails(materialType, purity) {
       ...
       function calculatePurchaseAmount() {
        ...
        const marketRate24K = parseFloat(stockRate.value) || 0;
        const actualPurityPercent = getCurrentPurityValue();
        const itemBuyingRatePerGram = parseFloat(buyingPurity.value) || 0;
        ...
        if (weight > 0) {
            if (isPurchaseChecked) {
                ...
                if (marketRate24K > 0 && actualPurityPercent > 0) {
                    materialCost = weight * marketRate24K * (actualPurityPercent / 99.99);
                    calculatedTotalValue = materialCost;
                }
            }
        }
        ...
        if (isPurchaseChecked) {
            ...
            let status = 'Due';
            if (calculatedTotalValue > 0 && paidAmountValue > 0) {
                status = paidAmountValue >= calculatedTotalValue ? 'Paid' : 'Partial';
            } else if (calculatedTotalValue <= 0 && paidAmountValue <= 0) {
                status = 'Due';
            } else if (paidAmountValue === 0 && calculatedTotalValue > 0) {
                status = 'Due';
            } else if (paidAmountValue > 0 && calculatedTotalValue === 0) {
                status = 'Error';
            }
            ...
            if (balance < 0) {
                balanceAmount.textContent = '₹0.00 (Overpaid)';
            }
        } else {
      ...
      function handleAddStock() {
          ...
          // For purchases, send the buying rate as the 'rate' field
          // For non-purchases, send the market rate
          if (isPurchase.checked) {
              formData.append('rate', buyingPurity.value);
          } else {
          ...
          .then(data => {
              if(data.success) {
                  ...
                  if(purity) {
                      ...
                      // If purity is not set after reset, clear details
                      resetStockDetails();
                  }
              } else {
      ...
      function validateFields() {
          ...
          if(!stockWeight.value || (!stockRate.value && !isPurchase.checked)) {
              alert('Please enter weight and rate (or buying purity for purchase)');
              return false;
          }
          ...
          if(isPurchase.checked) {
              ...
              if(!buyingPurity.value) {
                   // Check buying purity only if it's a purchase
                  alert('Please enter buying purity');
                  return false;
              }
              // Payment mode is only required if paid amount is greater than 0
      ...
      function appendPurchaseData(formData) {
        const weight = parseFloat(stockWeight.value) || 0;
        const buyingRate = parseFloat(buyingPurity.value) || 0;
        const totalAmount = weight * buyingRate;
        ...
        // Only append payment_mode if a paid amount is entered and a mode is selected
      ...
      function updateSuppliersList(suppliers, selectSupplierId = null) {
          ...
          if (selectSupplierId) {
              supplier.value = selectSupplierId;
          }
      }
...
function calculateBuyingRate() {
  ...
  const materialType = stockMetalType.value; // Get the selected material type

  if(baseRate > 0 && purity > 0 && materialType) {
      let finePurity = 0;
      if (materialType === 'Gold') {
          ...
          buyingPurity.value = ''; // Clear buying rate if material type is not Gold or Silver
          return;
      }

      if (finePurity > 0) {
           // Calculate buying rate based on purity ratio relative to the 'fine' purity rate
          const buyingRate = baseRate * (purity / finePurity);
          buyingPurity.value = buyingRate.toFixed(2);
      } else {
           buyingPurity.value = ''; // Clear buying rate if fine purity is not determined
      }
  } else {
      buyingPurity.value = ''; // Clear buying rate if inputs are invalid
  }
}
...
// Quick Add Supplier Modal Functionality
const addSupplierBtn = document.getElementById('addSupplierBtn');
const addSupplierModal = document.getElementById('addSupplierModal');
const closeSupplierModal = document.getElementById('closeSupplierModal');
const quickAddSupplierForm = document.getElementById('quickAddSupplierForm');
...
const newSupplierGst = document.getElementById('newSupplierGst');
...
quickAddSupplierForm.addEventListener('submit', function(e) {
  e.preventDefault();
  handleAddSupplier();
});
...
function handleAddSupplier() {
  ...
  const gst = newSupplierGst.value.trim();
...
// Supplier Autosuggestion Functionality
const supplierInput = document.getElementById('supplier'); // We will treat the select as an input for suggestion for now
const supplierSuggestions = document.createElement('div'); // Create a div for suggestions
supplierSuggestions.className = 'suggestions-container absolute w-full mt-1';
supplierInput.parentNode.style.position = 'relative'; // Set parent to relative for absolute positioning
supplierInput.parentNode.appendChild(supplierSuggestions); // Append suggestions div
...
// Modify loadSuppliers to accept an optional supplierId to select after loading
function loadSuppliers(selectSupplierId = null) {
  fetch('stock_functions.php', {
      method: 'POST',
      headers: {
...
// Helper function to get current purity value
function getCurrentPurityValue() {
    const purity = stockPurity.value === 'custom' ? customPurity.value : stockPurity.value;
    return parseFloat(purity) || 0;
}